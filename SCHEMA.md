...
The public edition uses schema version 1. Files ending in `.jsonl` contain one UTF-8 JSON object per line. Join passage and source records by `video_id`; use `chunk_key` as the stable passage identifier. Times are seconds in the source video. Upload dates use `YYYY-MM-DD`.
...
- `start_seconds`, `end_seconds`: approximate passage bounds. `timing_method` describes alignment limits.
- `text`: corrected reading text, copied without changing its characters or whitespace. A single record has empty text because neither model detected speech in the on-screen guide.
...
- `alternatives`: local Whisper large-v3 text, plus Parakeet and YouTube-caption comparisons for the pilot where available. Alternatives may be wrong and may have imperfect timing alignment. Repeated experimental short-chunk API outputs are not included.
- `text_disagreement_heuristic`, `heuristic_note`: disagreement diagnostic, not an accuracy measurement, probability or word error rate.
- `critical_disagreements`: candidate wording, local alternative, heuristic flags and review notes for potentially important differences. These can include false positives.
...
- `review_notes`: scoped editorial comparison notes. A resolved sentence does not resolve all other issues in its passage.
...
- `source_audio_sha256`, `provenance`: hashes identifying the underlying audio and transcription artifacts. Local filesystem paths and API request identifiers are excluded. Whisper model repository, revision and precision are retained where available.
